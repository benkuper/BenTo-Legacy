...
public :
  ...
  IMUManager()
  {
   addCallbackOrientationUpdate(&IMUManager::defaultCallback);
  }
  
  void init()
  {
    #if SERIAL_DEBUG
      Serial.println("IMUManager init.");
    #endif
  ...
  void update()
  {
    ...
    //TODO, implement magbias calibration
   ...
   onOrientationUpdate(yaw,pitch,roll);
  }

  
  typedef void(*onEvent)(float, float, float);
  void (*onOrientationUpdate) (float, float, float);
  
  void addCallbackOrientationUpdate (onEvent func) { 
    onOrientationUpdate = func;
  }

  static void defaultCallback(float yaw, float pitch, float roll)
  {
    //nothing
  }
  ...
  void magcalMPU9250(float * dest1, float * dest2) 
  {
     ...
     for(ii = 0; ii < sample_count; ii++) {
       ...
       for (int jj = 0; jj < 3; jj++) {
        ...
        delay(30);  // at 8 Hz ODR, new mag data is available every 125 ms
     }
